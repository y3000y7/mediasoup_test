...
const MediaSettings = ({
  ...
  if (me.webcamDevices) webcams = Object.values(me.webcamDevices);
  else webcams = [];

  let audioDevices;

  if (me.audioDevices) audioDevices = Object.values(me.audioDevices);
  else audioDevices = [];
        ...
        <FormControl className={classes.formControl}>
          ...
          >
            {audioDevices.map((audio, index) => {
              return (
                <MenuItem key={index} value={audio.deviceId}>
                  {audio.label === "" ? index + 1 : audio.label}
                </MenuItem>
              );
            })}
          </Select>
            ...
            <FormControl className={classes.formControl}>
              ...
              >
                {audioOutputDevices.map((audioOutput, index) => {
                  return (
                    <MenuItem key={index} value={audioOutput.deviceId}>
                      {audioOutput.label}
                    </MenuItem>
                  );
                })}
          ...
          <Collapse in={audioSettingsOpen} timeout="auto">
            <List component="div">
              ...
              <ListItem className={classes.nested}>
                ...
                <NoiseSlider
                  className={classnames(classes.slider, classnames.setting)}
