...
class Room extends EventEmitter {
  ...
  static async create({ mediasoupWorkers, roomId, peers }) {
    ...
    const firstRouter = mediasoupRouters.get(
      Room.getLeastLoadedRouter(mediasoupWorkers, peers, mediasoupRouters)
    );
  ...
  async _handleSocketRequest(peer, request, cb) {
    ...
    switch (request.method) {
      ...
      case "drawLines": {
        if (!this._hasPermission(peer, DRAW_LINES))
          throw new Error("peer not authorized");

        const { lines } = request.data;

        this._drawHistory.push(lines);
